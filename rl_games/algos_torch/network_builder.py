...
class A2CBuilder(NetworkBuilder):
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            self.actor_mlp = self._build_mlp(**actor_args)
            if self.separate:
                self.critic_mlp = self._build_mlp(**critic_args)
        ...
        def load(self, params):
            ...
            self.critic_units = params.get("critic_mlp", {"units": self.units})["units"]
            self.activation = params["mlp"]["activation"]
            self.critic_activation = params.get(
                "critic_mlp", {"activation": self.activation}
            )["activation"]
